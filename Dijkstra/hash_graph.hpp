...
HashGraph<T>::HashGraph (const HashGraph& g) {
        this->node_values = g.node_values;
        this->edge_values = g.edge_values;
}
...
bool HashGraph<T>::has_node(NodeName node_name) const {
        ...
        else if(node_values.has_key(node_name))
        {
            return true;
        }
}
...
bool HashGraph<T>::has_edge(NodeName origin, NodeName destination) const {
        auto item = ics::make_pair(origin,destination);
        if (!edge_values.has_key(item))
        {
            return false;
        }
        else if(edge_values.has_key(item))
        {
            return true;
        }
}
...
T HashGraph<T>::edge_value(NodeName origin, NodeName destination) const {
        ...
        {
            auto item = ics::make_pair(origin, destination);
            return edge_values[item];
        }
        else if(!has_edge(origin,destination))
        {
...
int HashGraph<T>::in_degree(NodeName node_name) const {
        ...
        {
            int result = node_values[node_name].in_nodes.size();
            return result;
        }
        else if(!has_node(node_name))
        {
            throw GraphError("ERROR");
        }
}
...
int HashGraph<T>::out_degree(NodeName node_name) const {
        if (has_node(node_name))
        {
            int result = node_values[node_name].out_nodes.size();
            return result;
        }
        else if(!has_node(node_name))
        {
...
void HashGraph<T>::add_node (NodeName node_name) {
        backup_node[node_name] = LocalInfo();
        node_values[node_name] = LocalInfo();
}
...
void HashGraph<T>::add_edge (NodeName origin, NodeName destination, T value) {
        auto item = ics::make_pair(origin,destination);
        edge_values[item] = value;
        node_values[origin].out_nodes.insert(destination);
        node_values[origin].out_edges.insert(item);
        node_values[destination].in_nodes.insert(origin);
        node_values[destination].in_edges.insert(item);
}
...
void HashGraph<T>::remove_node (NodeName node_name){
        if(!has_node(node_name))
        {
            backup_node.erase(node_name);
        }
        else if(has_node(node_name))
        {
            auto data = node_values[node_name];
            for(auto kv: data.in_edges)
            {
                edge_values.erase(kv);
                node_values[kv.first].out_edges.erase(kv);
                node_values[kv.first].out_nodes.erase(node_name);
            }
            for(auto kv:data.out_edges)
            {
                edge_values.erase(kv);
                node_values[kv.second].in_edges.erase(kv);
                node_values[kv.second].in_nodes.erase(node_name);
            }
...
void HashGraph<T>::remove_edge (NodeName origin, NodeName destination) {
        if(!has_edge(origin, destination))
        {
            auto data=ics::make_pair(origin,destination);
            backup_node[origin].out_edges.erase(data);
            backup_node[origin].in_edges.erase(data);
        }
        else if(has_edge(origin, destination))
        {
            auto data=ics::make_pair(origin,destination);
            edge_values.erase(data);
            node_values[origin].out_edges.erase(data);
            node_values[destination].in_edges.erase(data);
...
void HashGraph<T>::load (std::ifstream& in_file, std::string separator) {
        ...
        {
            auto data = split(s,separator);
            if (data.size() != 1)
            {
                add_edge(data[0],data[1],atoi(data[2].c_str()));
            }
            else if(data.size() == 1)
            {
                add_node(data[0]);
...
void HashGraph<T>::store(std::ofstream& out_file, std::string separator) {
        ...
        {
            out_file << kv.first.first;
            out_file << separator;
            out_file << kv.first.second;
            out_file << separator;
            out_file << kv.second;
            out_file << std::endl;
        }
...
bool HashGraph<T>::operator == (const HashGraph<T>& rhs) const{
        ...
        if(edge_values.size() != rhs.edge_values.size() || node_values.size()!=rhs.node_values.size())
        {
            return false;
        }
        for(auto kv:node_values)
        {
            if(!rhs.node_values.has_key(kv.first))
            {
                return false;
            }
        }
        for(auto kv:edge_values)
        {
            ...
            if(!rhs.edge_values.has_key(kv.first) || rhs.edge_values[kv.first]!=kv.second)
            {
                return false;
            }
        }
...
std::ostream& operator<<(std::ostream& outs, const HashGraph<T>& g) {
    std::cout<< "graph[";
    std::cout<< std::endl;
    for(auto kv: g.node_values)
    {
        auto data = kv.second.out_edges.begin();
        auto item = kv.second.in_edges.begin();
        std::cout<< kv.first;
        std::cout<< " -> LocalInfo[";
        std::cout<< std::endl;
        std::cout<< "       out_nodes = ";
        std::cout<< kv.second.out_nodes;
        std::cout<< std::endl;
        std::cout<< "       out_edges = set[";
        std::cout<<"->";
        std::cout<<data->second;
        std::cout<<"(";
        std::cout<<g.edge_values[*data];
        std::cout<<")";
        data++;
        while(data != kv.second.out_edges.end())
        {
            std::cout<< ",->";
            std::cout<< data->second;
            std::cout<< "(";
        ...
        std::cout << kv.second.out_nodes;
        std::cout << std::endl;
        std::cout << "       in_edges = set[";
        std::cout << item->first;
        std::cout << "("<<g.edge_values[*item];
        std::cout << ")->";
        item++;
        while(item != kv.second.in_edges.end())
        {
            std::cout << ",";
